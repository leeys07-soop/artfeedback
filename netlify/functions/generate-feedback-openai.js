exports.handler = async function(event, context) {
  ...
  if (!OPENAI_API_KEY) {
    return { statusCode: 500, body: JSON.stringify({ error: 'OpenAI API Key가 설정되지 않았습니다.' }) };
  }

  try {
    const body = JSON.parse(event.body);
    const { name, s1, s2, s3, s4 } = body;
...
문장을 중간에 끊지 않고 반드시 완성합니다.`
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) throw new Error('OpenAI API Key가 올바르지 않습니다.');
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: e.message || '서버 오류가 발생했습니다.' })
    };
  }
};
