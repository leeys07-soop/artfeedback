...
아래 예시와 같은 문체와 느낌으로 작성해주세요. 단, 예시에 나오는 이름(민영이, 지율이)은 절대 사용하지 말고 반드시 위에 적힌 아이 이름(${childName})만 사용하세요.
...
인사말이나 서명 없이 피드백 본문만 작성해주세요.`;
    ...
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`,
      {
        ...
        body: JSON.stringify({
          systemInstruction: {
            parts: [{ text: `당신은 10년 경력의 미술학원 선생님입니다.
학부모님께 수업 피드백을 전달할 때 주어를 생략하고 자연스럽게 씁니다. "선생님이~" 같은 표현은 쓰지 않습니다.
예: "색을 고르는 안목이 특별하다고 인정해주었을 때~", "수업 중 조심스러워하는 마음에 공감하며 응원해주었습니다."
...
문장을 중간에 끊지 않고 반드시 완성합니다.` }]
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        return res.status(429).json({ error: '현재 Gemini 서버가 혼잡합니다. OpenAI 버전을 사용해주세요.' });
      }
